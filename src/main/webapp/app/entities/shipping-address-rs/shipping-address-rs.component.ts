...
@Component({
  selector: 'jhi-shipping-address-rs',
  templateUrl: './shipping-address-rs.component.html'
})
export class ShippingAddressRsComponent implements OnInit, OnDestroy {
  currentAccount: any;
  ...
  loadAll() {
    this.shippingAddressService
      ...
      .subscribe(
        (res: HttpResponse<IShippingAddressRs[]>) => this.paginateShippingAddresses(res.body, res.headers),
  ...
  transition() {
    this.router.navigate(['/shipping-address-rs'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  trackId(index: number, item: IShippingAddressRs) {
    return item.id;
  }
  ...
  protected paginateShippingAddresses(data: IShippingAddressRs[], headers: HttpHeaders) {
